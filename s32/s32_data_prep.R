...
write.csv(indep_cols, "s32_indep.csv", row.names=F)
...
write.csv(generator, "s32_generator.csv", row.names=F)
...
write.csv(unique_lines, "s32_lines.csv", row.names=F)
...
saturated <- 1:31
catalogue <- read.csv("s32_catalogue.csv")
orig_good_A_idx <- c()
comp_good_A_idx <- c()
...
write.csv(good_A, "s32_good_A.csv", row.names = FALSE)
