...
corresponding_bi <- list()
for (i in 1:40) {
  bi <- c()
  for (j in 1:nrow(form_line)) {
    if (i %in% form_line[j, ]) {
      bi <- c(bi, form_line[j, -which(form_line[j, ] == i)])
    }
  }
  bi <- unique(bi)
  corresponding_bi[[i]] <- bi
}

factor_add <- function(a, b) {
  add_up <- S81[, a] + S81[, b]
  if (!first_is_one(add_up)) add_up <- (add_up * 2) %% 3
  return(mapping[as.character(add_up %*% 3 ^ (0:3))])
}

ai <- read.csv("s81_good_A.csv")
